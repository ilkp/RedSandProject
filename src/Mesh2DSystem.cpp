...
void Mesh2DSystem::copy(Entity entity, const Mesh2D& mesh)
{
	...
	copy(entity, mesh.vertices, mesh.verticesLength, mesh.triangles, mesh.trianglesLength);
	...
	const unsigned int* triangles, unsigned int trianglesSize)
{
	if (!indices.contains(entity))
		reserve(entity);
	Mesh2D& internalMesh = data[indices[entity]];
	internalMesh.verticesLength = verticesSize;
	internalMesh.trianglesLength = trianglesSize;
	internalMesh.vertices = new float[internalMesh.verticesLength];
	internalMesh.triangles = new unsigned int[internalMesh.trianglesLength];
...
void Mesh2DSystem::calculateBounds(Entity entity, unsigned int stride)
{
	...
	for (unsigned i = 0; i < mesh.verticesLength / stride; i += stride)
	{
